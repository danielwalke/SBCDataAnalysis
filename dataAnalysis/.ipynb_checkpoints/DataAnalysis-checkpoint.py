...
class DataAnalysis:
    def __init__(self, data=None, print_logs = False, mimic_data = None):
        ...
        if print_logs:
            ...
            print(f"Control data are {count_cbc_cases(self.greifswald_vaidation.get_control_data())} cases "
        ...
        if print_logs and mimic_data is not None:
            print(f"Controls Mimic: {self.mimic.get_control_data().shape[0]},"
              f" Sepsis Mimic: {self.mimic.get_sepsis_data().shape[0]}")
            print(f"Assessable data Mimic are {count_cbc_cases(self.mimic.get_data())} cases "
                  f"and {count_cbc(self.mimic.get_data())} CBCs")
            print(f"Control data Mimic are {count_cbc_cases(self.mimic.get_control_data())} cases "
                  f"and {count_cbc(self.mimic.get_control_data())} CBCs")
            print(f"Sepsis data Mimic are {count_cbc_cases(self.mimic.get_sepsis_data())} cases "
